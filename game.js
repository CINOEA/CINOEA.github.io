...
var jump = {
    ...
    isGrounded: true,
    up: function () {
        if (hand.y > 20+this.startPosition/*(height * (height / width) - hand.h )*/ - this.h * (height / 662) && this.isGrounded) {
            hand.y -= this.speed;
            hand.drawFrames(4, 4);
        }
    ...
    down: function () {
        if (hand.y < 20+this.startPosition/*(height * (height / width) - hand.h )*/) {
            hand.y += this.speed;
            hand.drawFrames(2, 2);
            this.isGrounded = false;
        }
        else {
            hand.y = 20+this.startPosition/*height * (height / width) - hand.h*/;
            this.state = "up";
